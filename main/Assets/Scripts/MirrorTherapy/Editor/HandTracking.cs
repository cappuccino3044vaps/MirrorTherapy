...
[CustomEditor(typeof(CREATE_Tracking))]
[CanEditMultipleObjects]
class CREATE_Tracking_Editor : Editor {
  ...
  public override void OnInspectorGUI() {
    ...
    foreach (CREATE_Tracking target in targets) {
    ...
    GUILayout.Space(5);
    if (GUILayout.Button("Auto detect properties")) AutoDetect(target as CREATE_Tracking);
    EditorGUILayout.HelpBox(
        "Please manually check if auto detect set fields correctly. Please refer to help page for details.",
        MessageType.Info);
  }

  private void AutoDetect(CREATE_Tracking target) {
  ...
  private void SetBone(CREATE_Tracking target, string name, int boneIndex, int count) {
    var bones =
  ...
  private int CountName(CREATE_Tracking target, params string[] names) {
    int maxCount = 0;
