...
{
[CustomEditor(typeof(VRM_handtracking_mirror))]
[CanEditMultipleObjects]
class VRM_handtracking_mirror_Editor : Editor
{
    ...
    SerializedProperty rotationProp, isLeftProp, handProp, nodesProp, mirrorProp,collliderProp, confidenceProp
    ,MirrorTagProp;
    AnimBool showNodes, showMirrorNodes;
    ...
    void OnEnable(){
        ...
        MirrorTagProp=serializedObject.FindProperty("MirrorTag");
        nodesProp=serializedObject.FindProperty("Nodes");
        showNodes = new AnimBool(true);
        showNodes.valueChanged.AddListener(Repaint);
        mirrorProp=serializedObject.FindProperty("MirrorNodes");
        showMirrorNodes = new AnimBool(true);
        showMirrorNodes.valueChanged.AddListener(Repaint);
    }
    public override void OnInspectorGUI(){
        ...
        EditorGUILayout.PropertyField(MirrorTagProp);
    ...
    GUI.enabled = true;
    if (mirrorProp.hasMultipleDifferentValues)
      EditorGUILayout.LabelField("MirrorNodes", "-");
    else {
      showMirrorNodes.target = EditorGUILayout.Foldout(showMirrorNodes.target, "MirrorNodes");
      if (EditorGUILayout.BeginFadeGroup(showMirrorNodes.faded)) {
        EditorGUI.indentLevel++;
        for (int i = 0; i < 21; i++) {
          var element = mirrorProp.GetArrayElementAtIndex(i);
          element.objectReferenceValue = EditorGUILayout.ObjectField(
              names[i], element.objectReferenceValue, typeof(Transform), true);
        }
        EditorGUI.indentLevel--;
    ...
    //モデルオブジェクトの値を変更する
    foreach (VRM_handtracking_mirror target in targets) {
    ...
    if (GUILayout.Button("Auto detect properties")) AutoDetect(target as VRM_handtracking_mirror);
    EditorGUILayout.HelpBox(
        "自動的にNodeを設定します。対象となるモデルによっては正しく設定できない場合があります。気を付けてくださいね。",
        MessageType.Info);
    }
    private void AutoDetect(VRM_handtracking_mirror target){
    ...
    private void SetBone(VRM_handtracking_mirror target, string name, int boneIndex, int count) {
        var bones =
    ...
    private int CountName(VRM_handtracking_mirror target, params string[] names) {
        int maxCount = 0;
